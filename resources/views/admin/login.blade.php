...
@section('content')
    {{-- ここが実際にページごとに変わる部分 --}}
    <p class="text-center">アドミン用のログイン</p>
    @php
        $inputClass = 'w-full border rounded px-3 py-2';
    @endphp

    <form method="POST" action="{{ route('admin.login') }}" class="max-w-sm mx-auto mt-10 space-y-4">
        @csrf
        ...
        <button class="w-full bg-red-500 text-white py-2 rounded">ログイン</button>
    </form>
@endsection
